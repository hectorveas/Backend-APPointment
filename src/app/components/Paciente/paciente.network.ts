...
router.get("/rut", async function (req: Request, res: Response) {
  ...
  try {
    const paciente: Paciente | null = await pacienteController.BuscarPacienteRut(
      req.body.rut
    );
    if (paciente != null) {
      responseModule.success(req, res, paciente, 200);
...
router.patch("/:id", async function (req: Request, res: Response) {
  ...
  try {
    const modPaciente = await pacienteController.modificarPaciente(
      id,
      req.body
    );
    if (modPaciente != null) {
      responseModule.success(req, res, modPaciente, 200);
    } else {
      responseModule.success(req, res, "NO SE ENCONTRO AL PACIENTE");
    }
  } catch (error) {
